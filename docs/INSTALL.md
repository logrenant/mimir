...
Mimir is a zero-cost, local Model Context Protocol (MCP) server that empowers your Claude Code session with the ability to search and read the web.
...
mimir-mcp calls `claude` headlessly (`-p`, no tools, no session persistence) to refine scraped pages — it rides this login, so no separate API key is needed. Model `claude-haiku-4-5-20251001` is pinned in `internal/config`.

## Building the Binary

Compile the Mimir binary:

```bash
make build
```

This will produce the executable at `bin/mimir-mcp`.

## Always-on install (the normal one)

Mimir is meant to run whether or not a window is open. Two pieces do that, and
they are installed separately:

```bash
make install-agent     # mimir-daemon as a launchd agent: at login, and restarted if it dies
make desktop-build     # builds Mimir.app (menu-bar app; first run compiles Rust)
...
1. builds `bin/mimir-daemon` and installs it to
   `~/Library/Application Support/mimir/bin/`;
...
3. writes `endpoint.json` (`0600`) for the app to read, and
   `~/Library/LaunchAgents/studio.mimir.daemon.plist` (`0600`) for launchd;
4. loads the agent and waits for `/healthz`.

Open `Mimir.app` once. It has **no Dock icon** — it is a menu-bar app. From its
menu-bar item: `New task…` (or **⌘⇧G** from anywhere) opens the quick-task
window, `Open Mimir` brings back the main window, and `Start Mimir at login`
registers the app itself as a login item. Closing a window hides it; `Quit Mimir`
is the only way out, and it leaves the daemon running.

| Command | What |
|---------|------|
| `make agent-status` | is the agent loaded, and with which pid |
| `make agent-logs`   | follow `~/Library/Logs/mimir-daemon.log` |
...
it picks a free loopback port, mints a per-launch token, runs `mimir-daemon` as
...
```bash
make desktop-sidecar   # builds mimir-daemon into the app's sidecar slot
...
automation permission for its AppleScript layout step, and Mimir is installed by
...
Once registered, start or reload your Claude Code session. You can verify that Mimir is healthy by asking Claude to run the `diagnostics` tool:

> **You:** "Please run the mimir-mcp diagnostics tool to check if the services are healthy."
...
| `mapscrape: google served a consent or block page` | Google answered with a consent wall or captcha rather than results. | Not a code failure and not something Mimir works around. Try again later, or use the Places API path. |
...
| ⌘⇧G does nothing | Another app owns the shortcut, or two copies of Mimir are running. | Use the menu-bar item's `New task…`; quit the extra copy (a `make desktop-dev` session counts). |
