...
// Version is the Mimir binary version, surfaced by the diagnostics tool, by
...
// NewServer constructs an SDK server with name 'mimir-mcp', a version string, and stdio transport.
...
// the daemon cannot expose a tool set that has drifted from mimir-mcp's, because
// there is only one.
...
func (s *Server) Run(ctx context.Context) error {
	slog.Info("Starting MCP server", "name", "mimir-mcp", "version", version)
