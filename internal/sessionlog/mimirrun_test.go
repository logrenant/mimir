...
func TestParseMimirRun(t *testing.T) {
	...
	ep, err := ParseMimirRun(strings.NewReader(transcript),
		Source{Kind: SourceMimirRun, Path: "/t/r1.jsonl", ProjectPath: "/work/demo"},
		RunMeta{RunID: "r1", Prompt: "add a retry to the crawl client"})
	if err != nil {
		t.Fatalf("ParseMimirRun: %v", err)
	}

	if ep.Key != "run:r1" {
		t.Errorf("Key = %q", ep.Key)
	}
	if ep.SourceKind != SourceMimirRun || ep.SessionID != "cli-sess" {
...
func TestParseMimirRun_AbsentOKIsNotAFailure(t *testing.T) {
	transcript := runTranscript(t,
	...
	ep, err := ParseMimirRun(strings.NewReader(transcript),
		Source{Kind: SourceMimirRun, ProjectPath: "/work/demo"}, RunMeta{RunID: "r1", Prompt: "p"})
	if err != nil {
		t.Fatalf("ParseMimirRun: %v", err)
	}
...
func TestParseMimirRun_FailedRunRecordsItsError(t *testing.T) {
	transcript := runTranscript(t,
...
func TestParseMimirRun_JunkIsSkipped(t *testing.T) {
	input := "not json\n{\"kind\":\"unknown.kind\"}\n" + runTranscript(t,
		events.Event{Kind: events.KindTextDelta, RunID: "r1", Seq: 1, At: at(0), Text: "ok"})

	ep, err := ParseMimirRun(strings.NewReader(input),
		Source{Kind: SourceMimirRun, ProjectPath: "/work/demo"}, RunMeta{RunID: "r1", Prompt: "p"})
	if err != nil {
