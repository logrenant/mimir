...
| [task-71](task-71-blank-window.md) | done | The blank window: macOS suspends the WebView behind a hidden window and does not always bring it back — the shell now pokes it and reloads what does not answer, a crashed screen shows a message instead of nothing, and the Brain layout stops blocking the main thread. |
| [task-72](task-72-graphify-structural.md) | done | Graphify as an optional structural layer: symbols and their calls and imports, parsed deterministically and written with no model call, joined to the file nodes the scan already made. Absent on a machine without it, and silent about it. |
| [task-73](task-73-project-identity.md) | done | Moving a project and forgetting one: the first writes in this system that remove. A renamed repository left its old path in the graph for ever, re-recorded every five minutes from transcripts that still named it. |
| [task-74](task-74-chat-archive-coverage.md) | done | The verbatim archive covers every project the scan discovers, mimir-mcp writes to it, and the history recorded before the archive existed is read back in — with the recap budget moved to per pass so the wider coverage costs no more model calls. |
| [task-75](task-75-graphify-hardening.md) | done | Graphify stops re-parsing unchanged projects, its certain edges are handed to the relation pass instead of being re-guessed, the graph filters symbols out by default, and the symbol ceiling is visible. |
| [task-76](task-76-desktop-visual-language.md) | done | Desktop: tek görsel dil. İki uyumsuz stil dili — token'lar ve `hover.tsx`'in satır-içi hex'i — birleşti; yükselti ışığa, vurgu kayan bir raya, boş durum ve yükleniyor birer bileşene döndü. Fark edilmemiş beşinci renk ve odak halkasının yokluğu giderildi. |
| [task-77](task-77-skills-and-agents.md) | done | Skill deposu (gömülü varsayılan + operatörün diskteki kopyası), beş ajanlık defter, kartın ajanını seçen yönlendirici, ve iki sert kapı: skill yüklenemeyen iş çalışmaz. |
| [task-78](task-78-desktop-skills.md) | done | Desktop: kartta ajan seçici ve skill rozetleri; Ayarlar'da skill editörü. `modules.ts` elle yazılmış liste olmaktan çıkar. |
| [task-79](task-79-executor-seam.md) | done | Executor dikişi: dispatcher claude dışında bir şey de başlatabilir. Davranışı değişmeyen bir refactor — tek kayıtlı executor hâlâ claude. |
| [task-80](task-80-desktop-board.md) | done | Desktop: board tek istekle doluyor (1+N fan-out silinir), kartta ajan çipi. |
| [task-81](task-81-leadgen-executor.md) | done | Lead-gen bir alt-ajan: hesap istemeyen ilk executor, kuyruğa giren kart, aşamaları akan terminal. `runner.go`'ya dokunmaz. |
| [task-82](task-82-desktop-leadgen-card.md) | done | Desktop: lead-gen kartının gövdesi (bölge, sorgu, şirket sayısı) ve sonucu sahibi ekrana açan kapı. |
| [task-83](task-83-graph-harvest.md) | done | Graphify'dan alınmayan verim: uzantı tavanı 18→102, kenar yönünü koruyan okuma, beş traversal (query/path/explain/affected/god-nodes), dört MCP aracı. |
| [task-84](task-84-desktop-graph-console.md) | done | Desktop: Brain ekranında sorgu konsolu — "kim çağırıyor" sorusu ilk kez cevaplanır. |
| [task-85](task-85-catalog-core.md) | done | Katalog çekirdeği: Shopify/IKAS lehçeleri, bayt bayt kayıpsız CSV dönüşü, RTE zarfı (çıktı markanın etiket sözlüğünü genişletemez), ve CSV'nin kendisinden çıkarılan marka kimliği. |
| [task-86](task-86-desktop-catalog.md) | done | Desktop: Katalog ekranı — içe aktarma, ürün tablosu, eski/yeni önizleme, ve marka sözlüğüne kısıtlanmış zengin metin editörü. |
| [task-87](task-87-catalog-rewrite.md) | done | Ürün başına rakip araştırması ve yeniden yazım: iki fazlı önbellek (marka sesini düzeltmek araştırmayı atmaz), ve yarıda kalınca neyin korunduğunu söyleyen bir kart. |
| [task-88](task-88-desktop-catalog-card.md) | done | Desktop: katalog kartının gövdesi ve park rozeti — park edilmiş bir kart artık "hiç başlamadı"dan ayırt ediliyor, ve seçilenleri yeniden yazma düğmesi gerçek. |
| [task-89](task-89-executor-park.md) | done | `Outcome.ParkUntil` dikişte okunur: bir kimlik yuvası tutmayan işin limiti neye anahtarlanır, ve park edilmiş bir kart `queued`'dan nasıl ayırt edilir. |
| [task-90](task-90-desktop-catalog-mapping.md) | done | Desktop: eşleme kapısı `readable`'a bağlandı (kaydedilen eşleme ekranı açmıyordu), form önerilerle ve dosyanın kendi örnek değerleriyle açılıyor, ve bir ürün tam genişlikte açılıyor — editör solda, canlı önizleme sağda, `önce · sonra` anahtarıyla. |
| [task-91](task-91-catalog-dialects.md) | done | IKAS lehçeleri gerçek dışa aktarımlardan yeniden yazıldı (task-85'inki hafızadan yazılmıştı ve hiçbir gerçek dosyaya uymuyordu), özel alanlar dışa aktarımı tanınıyor, tanınmayan başlıklar için model çağrısız sütun önerisi, ve yeniden yazım her varyant satırına yazılıyor. |
| [task-92](task-92-desktop-brain-gestures.md) | done | Desktop: grafik kaydırmayı sahipleniyor (React'in pasif `wheel`'i yüzünden tek hareket hem grafiği hem sayfayı oynatıyordu), ve düğüm panelindeki düzyazı sarıyor — bir oturum URL'i panelin, panel de sayfanın genişliğini belirlemiyor. |
| [task-94](task-94-desktop-shell.md) | done | Desktop kabuğu: başarılı el sıkışma artık sessiz (poster ve "Devam" düğmesi gitti), ana ekrandan hesap paneli kalktı, ve menü çubuğunun native menüsü ile ekranın ortasındaki hızlı görev kutusu tek bir panele indi — ikonun altına tutturulmuş, uygulamanın kendi dilinde. |
| [task-93](task-93-llm-provider-registry.md) | done | Sağlayıcı kaydı: iki elle yazılmış sağlayıcı tek bir alt süreç çekirdeğine iner, kurulu CLI'lar keşfedilir, ve yapısal çıktı verip veremediği tipin parçası olur — şema isteyen bir iş, şema veremeyen bir sağlayıcıya yönlendirilemez. |
| [task-95](task-95-agent-cli-executors.md) | blocked | Ajan koşuları başka CLI'larda: `claude` bir spec'e iner, `gemini` ikinci ajan olur, ve kart hangi CLI'da koştuğunu satırında taşır. |
| [task-96](task-96-desktop-task-wizard.md) | done | Desktop: menü çubuğu paneli bir sohbete dönüşür — sihirbaz eksik olanı sorar, klasörü cümleden çıkarır ve çıkardığını görünür biçimde söyler. |
| [task-97](task-97-llm-connections.md) | done | Yönlendirmenin birimi bağlantı oldu: `agy` ile `gemini` aynı Google planından harcadığı için ikili adı kimlik olamaz. Ayrıca canlı bir hata kapandı — `Providers()` kayıtlı sağlayıcıların yarısını hiç sormuyor, dolayısıyla seçici onları bakmadan "sorunsuz" gösteriyordu. |
| [task-100](task-100-desktop-connections.md) | done | Desktop: bağlantılar bölümü ve bölüm rayı — `agy` ile `gemini` tek Google grubunda ve paylaşımı bir cümleyle söylüyor. Ayrıca task-98'in sınıf varsayılanları hiç kaydedilmiyordu; kural ikiye bölünmüştü, tek testli fonksiyona indirildi. |
| [task-99](task-99-secrets-and-api-adapters.md) | done | Sır kasası (OS deposu + `0600` dosya yedeği, ve **hangisine düştüğünü söyleyen** bir `Backend()`), on üç sağlayıcılık katalog, ve gerçek bir genişleme noktası: `POST /llm/connections` final gövdeyi alıyor ve hazır olmayanı adıyla reddediyor. Adaptörler, bir hesaba karşı sınanabildiklerinde. |
| [task-98](task-98-desktop-model-choice.md) | done | Desktop: modeli harcayan her yerde tek bir sağlayıcı seçicisi — ayarlardaki varsayılanlar, brain taraması, katalog, ve kart. |
| [task-113](task-113-catalog-per-language-approval.md) | done | Onay dil başına, ve dışa aktarım dosyanın taşıdığı **her** dili yazıyor — Arapça içerik ilk kez mağazaya gidebiliyor. Yanında operatörün taslak-kaydetme yolundaki üç sessiz kırık: kaybolan `dir="rtl"`, hiç çalışmayan dil kapısı, ve yanlış dile yazılan durum. |
| [task-114](task-114-catalog-outputs.md) | done | `GET /catalog/outputs`: yazılmış her taslak, bütün dosyaların üstünden, dile ve dosya profiline göre süzülebilir. Anahtarları stüdyo besteliyor, depo satır içi tablo olarak join ediyor, ve katalog ne kadar büyürse büyüsün tam iki okuma. |
| [task-115](task-115-desktop-catalog-outputs.md) | done | Desktop: eksik dil yüzeyleri (Arapça sekmesinde Arapça başlık, dile yazan onay, eşlenebilir diller), Kataloglar'ın yanında Çıktılar sekmesi, ve görsel iddiaların ölçülebilmesi için dev'e ait bir tarayıcı harness'ı. |
